...
const CTOCard: React.FC<CTOCardProps> = ({ style, title, children}) => {
  return (
    <Link to={'/'} className={`${style} overflow-hidden hover:overflow-visible relative group hover:-translate-y-8 cto_card rounded-3xl h-96 w-full md:min-w-[22rem] 2xl:max-w-[40rem] max-w-[24rem] p-5 flex flex-col justify-between delay-100 duration-200`}>
        {children}
